...
const C = {
  orange: "var(--brand-orange)", gold: "#F0C463", panel: "#12161d", panel2: "#171b23", edge: "#232a35",
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 10, color: C.grey }}>
      ...
      <div style={card}>
        ...
        <PotRow label="Entry sizes you allow">
        ...
        <Sect>How entries pay</Sect>
        <table style={tableStyle}>
          <thead><tr><Th>Entry</Th><Th r>Weight in the pot</Th></tr></thead>
          <tbody>
            {stakes.length === 0 && <tr><Td>—</Td><Td r color={C.dim}>Allow at least one entry size.</Td></tr>}
            {stakes.map((w) => (
        ...
        <div style={{ fontSize: 11, color: C.dim, lineHeight: 1.5, marginTop: 8 }}>
          One pot per slate. Players enter what they want; <b style={{ color: C.warn }}>payout scales with what you put up</b> and whether you were right.
      ...
      <div style={{ fontSize: 11, color: C.dim, textAlign: "center" }}>
        {canPublish ? (stakes.length ? "Every leg spans two games. Ready." : "Allow at least one entry size to publish.") : "Fix the flagged questions to publish."}
      </div>
    </div>
  );
}
...
function Td({ children, r, first, color }: { children: React.ReactNode; r?: boolean; first?: boolean; color?: string }) {
  return <td style={{ padding: "6px 0", borderBottom: "1px solid #1b212a", fontVariantNumeric: "tabular-nums", textAlign: r ? "right" : "left", color, boxShadow: first ? "inset 3px 0 0 var(--brand-orange)" : undefined }}>{children}</td>;
}
