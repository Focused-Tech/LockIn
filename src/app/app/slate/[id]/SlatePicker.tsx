    ...
    >
      <div className="sd-prow">
        <div>
          ...
          {/* DEMO shows coins, never $ — a demo can't win cash (Part 4). Live slates keep dollars. */}
          <span className="v">{isDemo ? formatCoinsShort(metrics.prizePoolCents) : formatCentsShort(metrics.prizePoolCents)}</span>
        </div>
        <div>
          <span className="k">1st place</span>
          <span className="v w">
            {isDemo ? formatCoinsShort(metrics.firstPlaceCents) : formatCentsShort(metrics.firstPlaceCents)}
